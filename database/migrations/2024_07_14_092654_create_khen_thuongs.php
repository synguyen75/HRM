...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('khen_thuongs', function (Blueprint $table) {
            ...
            $table->unsignedBigInteger('id_nguoi_khen_thuong')->nullable();
            $table->date('ngay_khen_thuong');
            $table->text('noi_dung');
            $table->timestamps();

            $table->foreign('id_nhan_vien')->references('id')->on('nhan_viens')->onDelete('set null');
            $table->foreign('id_nguoi_khen_thuong')->references('id')->on('nhan_viens')->onDelete('set null');
        });
    }
