...
@section('content')
    ...
    @endif
    <a href="{{ route('kyluat.create') }}" style="margin-bottom: 10px" class="btn btn-warning">Thêm phòng ban</a>
    <table class="table">
        ...
        <tbody>
            ...
            @foreach ($data as $key => $item)
                <tr>
                    ...
                    <td>{{ $item->nguoiKyLuat->ho_ten }}</td>
                    <td>{{ $item->nhanVien->ho_ten }}</td>
                    <td>{!! $item->noi_dung !!}</td>
                    <td>{{ $item->ngay_ky_luat }}</td>

                    <td>
                        <a href="{{ route('kyluat.edit', $item->id) }}" class="btn btn-warning"><i class="pe-7s-pen"
                                style="font-weight: bolder; font-size: 15px"></i></a>
                        <form action="{{ route('kyluat.destroy', $item->id) }}"
